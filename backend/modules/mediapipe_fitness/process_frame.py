# ProcessFrame for MediaPipe Fitness Trainer
...
class ProcessFrame:
    ...
    def get_current_feedback(self):
        """Returns the current feedback messages as a list of strings for voice"""
        feedback_messages = []
        
        if self.state_tracker['LOWER_HIPS']:
            feedback_messages.append('LOWER YOUR HIPS')
        
        for idx in np.where(self.state_tracker['COUNT_FRAMES'] > 0)[0]:
            if idx in self.FEEDBACK_ID_MAP:
                feedback_messages.append(self.FEEDBACK_ID_MAP[idx][0])
